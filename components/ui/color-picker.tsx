...
export const ColorPicker = ({ value, defaultValue = "#000000", onChange, className, ...props }: ColorPickerProps) => {
	...
	useEffect(() => {
		if (onChange) {
			...
			onChange([Number(rgba[0]), Number(rgba[1]), Number(rgba[2]), alpha / 100]);
		}
			...
			<div className={cn("-space-x-px flex items-center rounded-md shadow-sm", className)} {...props}>
				{rgb.map((value, index) => (
					<Input
						...
						key={["red", "green", "blue", "alpha"][index]}
						readOnly
						type="text"
						value={value}
					/>
			...
			<div className={cn("-space-x-px flex items-center rounded-md shadow-sm", className)} {...props}>
				{hsl.map((value, index) => (
					<Input
						...
						key={["hue", "saturation", "lightness", "alpha"][index]}
						readOnly
						type="text"
						value={value}
					/>
...
function resolveColor(value: Parameters<typeof Color>[0] | undefined, fallback: Parameters<typeof Color>[0]) {
	try {
