...
test("value conversion supports every target type", () => {
	expect(convertValue("42.5", "number")).toEqual({ ok: true, value: 42.5 });
	...
	expect(convertValue({ ok: true }, "text")).toEqual({ ok: true, value: '{"ok":true}' });
});
...
test("value conversion rejects lossy and mismatched values", () => {
	...
	expect(convertValue("0x10", "number").ok).toBe(false);
